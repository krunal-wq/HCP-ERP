...
# -------------------------------------------------------------------------------------
"""
fix_orphaned_child_permissions.py   (v2 â€” robust)
...
def main():
    with app.app_context():
            ...
            try:
                ...
                for up in list(ups):
                    ...
                    while current_parent_id and current_parent_id not in visited:
                        ...
                        if needs_fix:
                            print(f"  {u.username:<22} child='{mod.name}' â†’ enable parent='{parent.name}' ({reason})")
                            if APPLY:
                                if parent_up is None:
                ...
                if fixed_for_this_user:
                    ...
                    if APPLY:
                        try:
                            ...
                            db.session.rollback()
                            err = f"{u.username}: commit failed â€” {type(e).__name__}: {e}"
                            errors.append(err)
                            print(f"    âŒ {err}")
                            if VERBOSE:
                                traceback.print_exc()
                ...
                err = f"{u.username}: {type(e).__name__} â€” {e}"
                errors.append(err)
                print(f"    âŒ {err}")
                if VERBOSE:
                    traceback.print_exc()
        ...
        if APPLY and total_fixes:
            print(f"\n  âœ… Changes saved. User ko logout-login karna hoga.")
        elif APPLY:
            print(f"\n  âœ… Koi change zaroori nahi tha.")
        else:
