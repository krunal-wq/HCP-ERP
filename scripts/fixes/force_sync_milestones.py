...
Is script:
  ...
  2. Agar kisi project me same milestone_type ki DO rows hain (duplicates),
     to script ek row rakhega (jisme activity zyada ho â€” approved/data)
...
def activity_score(m):
    """Higher score = more activity on this row â†’ we keep this one when dedup."""
    score = 0
...
def main():
    with app.app_context():
        ...
        # â”€â”€ STEP 0: Sync template master â”€â”€
        ...
        for mtype, (title, icon, sort) in CORRECT.items():
            if mtype in existing:
                ...
                if t.title != title or t.icon != icon or t.sort_order != sort or not t.is_active:
                    print(f"   âœï¸  UPDATE template: {mtype} â†’ title='{title}' sort={sort}")
                    if APPLY:
        ...
        # â”€â”€ STEP 1: Per-project cleanup â”€â”€
        ...
        for p in projects:
            ...
            for r in list(rows):
                if r.milestone_type not in valid_types:
                    ...
                    print(f"  {p.code} âŒ DELETE invalid type: id={r.id} type={r.milestone_type} ({len(logs)} logs)")
                    if APPLY:
            ...
            # 1b. Group by milestone_type â€” dedupe duplicates
            by_type = {}
            for r in rows:
            ...
            for mtype, group in by_type.items():
                if len(group) > 1:
                    ...
                    for loser in losers:
                        ...
                        print(f"  {p.code} ðŸ”€ DEDUP  type={mtype}: keep id={winner.id}, delete id={loser.id} (move {len(logs)} logs)")
                        if APPLY:
                            for lg in logs:
            ...
            for r in rows:
                ...
                if r.title != correct_title:
                    print(f"  {p.code} âœï¸  FIX title: id={r.id} type={r.milestone_type}  '{r.title}' â†’ '{correct_title}'")
            ...
            for mtype, (title, _, sort) in CORRECT.items():
                if mtype not in present:
                    print(f"  {p.code} âž• ADD     {mtype}: {title}")
                    if APPLY:
        ...
        print(f"  is_selected NULL â†’ 0:      {stats['is_sel_fixed']}")
        print(f"  Missing rows added:        {stats['rows_added']}")

        if APPLY:
            print(f"\n  âœ… Changes committed. Ab refresh karo page aur Milestones tab dekho.")
        else:
            print(f"\n  â„¹ï¸  Dry-run. Output verify karke --apply se actually chalao.")
        print()
