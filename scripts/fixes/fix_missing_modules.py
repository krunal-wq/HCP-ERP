...
Yeh script safe hai â€” agar module already exist karta hai to skip kar dega.
"""
import sys, os
...
def ok(msg):  print(f"  {G}âœ… {msg}{E}")
def warn(msg): print(f"  {Y}âš ï¸  {msg}{E}")
def err(msg):  print(f"  {R}âŒ {msg}{E}")
...
with app.app_context():
    ...
    for m in MISSING_MODULES:
        ...
        if existing:
            warn(f"Module '{m['name']}' already exists (id={existing.id}) â€” skipping")
    ...
    # â”€â”€ RolePermissions add karo (sabhi roles ke liye) â”€â”€
    ...
    # â”€â”€ Existing UserPermissions update karo (admin users) â”€â”€
    ...
    for user in admin_users:
        for mod in added_modules:
            ...
            db.session.add(up)
            ok(f"UserPermission added for admin '{user.username}' â†’ '{mod.name}' + {len(sub_keys)} sub-perms")
    ...
    for m in MISSING_MODULES:
        ...
        if mod:
            ...
            err(f"'{m['name']}' â€” NOT FOUND in DB!")

    print(f"\n{G}  âœ… Done! Ab ACP panel mein Sample Orders aur Quotations ka{E}")
