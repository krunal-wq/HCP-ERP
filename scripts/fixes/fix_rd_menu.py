...
with app.app_context():
    ...
    # â”€â”€ STEP 1: R&D Trials â€” saare possible names se delete karo â”€â”€
    ...
    if mod:
        ...
        db.session.delete(mod)
        db.session.commit()
        ok(f"Trials module (label match) deleted âœ“")

    # â”€â”€ STEP 2: R&D parent module ensure â”€â”€
    ...
    # â”€â”€ STEP 3: Sub-modules define â”€â”€
    sub_modules = [
    ...
    for name, label, icon, url, sort in sub_modules:
        ...
        if not existing:
            ...
            db.session.commit()
            ok(f"'{label}' module added under R&D âœ“")
        else:
            # parent_id fix karo agar missing hai
            if existing.parent_id != rd.id:
                existing.parent_id = rd.id
                db.session.commit()
                ok(f"'{label}' parent_id fixed â†’ R&D âœ“")
            else:
                ok(f"'{label}' already exists âœ“")

    # â”€â”€ STEP 4: R&D Dashboard link bhi ensure karo â”€â”€
    rd.url_prefix = rd.url_prefix or '/rd'
    db.session.commit()

    print(f"\n{'='*55}")
    print(f"  {G}{B}âœ… DONE! Server restart karo.{E}")
    print(f"{'='*55}\n")
    print("  R&D menu ab aisa dikhega:")
    print("    ðŸ§ª R&D")
    print("      â”œâ”€â”€ R&D Dashboard")
    print("      â”œâ”€â”€ ðŸ—‚ï¸  R&D Projects")
    print("      â”œâ”€â”€ ðŸ“¦ Sample Ready")
    print("      â””â”€â”€ ðŸ“‹ Sample History\n")
