...
karta hai â€” woh script aap pehle se safely chala chuke ho.
...
2. Inserts default RolePermission rows for every role:
     admin â†’ full access ; everyone else â†’ all OFF (admin enables per-user).
3. Seeds UserPermission rows:
     â€¢ Admin users â†’ can_view/add/edit/delete/export = True
     â€¢ Non-admin   â†’ all False (admin selectively enables via ACP panel)
...
Phir browser mein /admin/user-permissions page kholo â€” Packing module
ab dikhega.
"""
import os
import sys
...
# â”€â”€â”€ Module config â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
PACKING_MODULE = {
    'name':       'packing',
    'label':      'Packing',
    'icon':       'ðŸ“¦',
...
# Role defaults â€” admin gets full rights, everyone else starts disabled
...
def seed_module():
    ...
    if existing:
        warn(f"Module 'packing' already exists (id={existing.id}) â€” skip insert")
    ...
    db.session.flush()
    db.session.commit()
    ok(f"Module added â†’ 'packing' (id={mod.id}, label='Packing')")
    return mod
...
def seed_role_perms(mod):
    ...
    for role, perms in ROLE_DEFAULTS.items():
        ...
        if existing:
            warn(f"RolePermission for role='{role}' already exists â€” skip")
            continue
        rp = RolePermission(
            role       = role,
            module_id  = mod.id,
        ...
        db.session.add(rp)
        ok(f"RolePermission seeded â†’ role='{role}' view={perms['view']}")
    db.session.commit()
...
def verify(mod):
    ...
    if not fresh:
        err("'packing' â€” NOT FOUND in DB after seeding!")
        return
