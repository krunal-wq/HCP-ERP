...
def main():
    with app.app_context():
        ...
        print(f"âœ”  {len(rd_ids)} active R&D users found: {sorted(rd_ids)}")
        print(f"Mode: {'APPLY (changes will be saved)' if APPLY else 'DRY-RUN (no changes)'}\n")

        # â”€â”€ 1. Deactivate bad RDSubAssignment rows â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        print(f"â”€â”€ RDSubAssignment rows to deactivate: {len(bad_subs)}")
        for s in bad_subs:
            ...
            print(f"   Â· {pcode:<12} variant={s.variant_code or 'â€”':<6} user={uname}")
            if APPLY:
                s.is_active = False

        # â”€â”€ 2. Clean legacy token fields on NPDProject â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        print()
        ...
        for p in projects:
            ...
            if p.assigned_rd_members:
                ...
                if new_val != p.assigned_rd_members:
                    print(f"   Â· {p.code}  assigned_rd_members: '{p.assigned_rd_members}'  â†’  '{new_val or ''}'")
                    cleaned_rd_members += 1
                    if APPLY:
                        p.assigned_rd_members = new_val

            # 2b. assigned_rd (single FK) â€” clear if user not in R&D
            if p.assigned_rd and p.assigned_rd not in rd_ids:
                ...
                print(f"   Â· {p.code}  assigned_rd: {uname}  â†’  (cleared)")
                cleared_assigned_rd += 1
                if APPLY:
                    p.assigned_rd = None

        print(f"\nâ”€â”€ assigned_rd_members rows cleaned: {cleaned_rd_members}")
        print(f"â”€â”€ assigned_rd (FK) rows cleared:    {cleared_assigned_rd}")

        if APPLY:
            db.session.commit()
            print("\nâœ… Changes committed to database.")
        else:
            print("\nâ„¹ï¸  Dry-run only. Re-run with --apply to save changes.")
