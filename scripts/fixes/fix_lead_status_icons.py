...
# -------------------------------------------------------------------------------------
"""
fix_lead_status_icons.py â€” lead_statuses table mein icons fix karo
...
print(f"\n{B}â”€â”€ Fixing lead_statuses icons...{E}\n")
...
statuses = [
    ("open",             "âœ‰ï¸",  "#6366f1"),
    ("in_process",       "âš™ï¸",  "#1e3a5f"),
    ("close",            "âœ…",  "#059669"),
    ("cancel",           "âŒ",  "#dc2626"),
    ("NPD Project",      "ðŸ§ª",  "#8b5cf6"),
    ("Existing Project", "ðŸ“¦",  "#0ea5e9"),
]
...
for name, icon, color in statuses:
    try:
        ...
        if cur.rowcount > 0:
            ...
            ok(f"{name} â†’ inserted with {icon}")
    except Exception as e:
        print(f"  {name}: {e}")
...
print(f"\n{G}{B}âœ… Done! Server restart karo.{E}\n")
