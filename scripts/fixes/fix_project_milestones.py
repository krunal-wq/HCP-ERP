...
# -------------------------------------------------------------------------------------
"""
fix_project_milestones.py â€” Milestone data cleanup aur diagnostic tool
...
Yeh script:
  ...
  3. Jo rows current templates me nahi hain, unhe DELETE karti hai
     (sirf rows jahan no activity hui â€” status='pending', no attachments,
      no notes, no approval â€” to purani usage ka data preserve rahe)
  4. Active rows (jinme kaam hua hai) ko sirf deselect karti hai â€” data
...
def main():
    with app.app_context():
        ...
        # â”€â”€ STEP 1: Fix npd_milestone_templates â”€â”€
        ...
        for t in templates:
            ...
            mark = "âœ… keep" if keep else "âŒ delete"
            print(f"   {mark}  {t.milestone_type:<22} â€” {t.title}")
            if not keep:
                to_delete.append(t)
        ...
        for mtype, title, icon, sort in CORRECT_MILESTONES:
            if mtype not in existing_types:
                ...
                print(f"   âž• insert  {mtype:<22} â€” {title}")

        print(f"\n  â†’ {len(to_delete)} templates to delete, {len(to_insert)} to insert")
        ...
        if APPLY:
            ...
            db.session.commit()
            print("  âœ… Template cleanup committed")

        # â”€â”€ STEP 2: Project-by-project milestone cleanup â”€â”€
        ...
        for p in projects:
            ...
            if VERBOSE:
                for m in good_rows:
                    sel = 'âœ“' if m.is_selected else 'âœ—'
                    print(f"       [{sel}] valid   â€” {m.milestone_type:<22} [{m.status}] {m.title}")

            for m in bad_rows:
                sel = 'âœ“' if m.is_selected else 'âœ—'
                if has_activity(m):
                    # Has real data â€” just deselect, don't delete
                    print(f"       [{sel}] KEEP    â€” {m.milestone_type:<22} [{m.status}] {m.title}  (has activity â†’ deselect only)")
                    if APPLY and m.is_selected:
                        m.is_selected = False
                    total_deselect += 1
                else:
                    # No real data â€” safe to delete
                    print(f"       [{sel}] DELETE  â€” {m.milestone_type:<22} [{m.status}] {m.title}  (no activity â†’ remove row)")
                    if APPLY:
                        db.session.delete(m)
                    total_delete += 1
            ...
            if missing:
                print(f"     âš ï¸  {len(missing)} standard milestone(s) missing â€” will add as deselected:")
                for mtype, title, icon, sort in missing:
                    print(f"       âž• add     â€” {mtype:<22} {title}")
                    if APPLY:
                        db.session.add(MilestoneMaster(
                            ...
                            title         = title,
                            sort_order    = sort,
                            is_selected   = False,   # off by default â€” user selects later
                            status        = 'pending',
                        ))
        ...
        print(f"  Bad rows to DESELECT:      {total_deselect}  (have activity â€” preserved)")

        if APPLY:
            print(f"\n  âœ… Changes committed to database.")
            print(f"  âž¡ï¸  Restart the app / refresh the Milestones tab.")
        else:
