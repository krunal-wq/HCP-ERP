...
=========================
One-time migration â€” adds the "R&D Sample Log" sub-menu under R&D.
...
------------
  ...
  2. Adds child Module `rd_sample_log` â†’ /rd/sample-log  (if missing).
  3. Seeds RolePermission rows so:
       - admin, manager, npd_manager, rd_manager  â†’ full CRUD
       - rd_executive, sales, hr, user            â†’ view only
  4. Idempotent â€” safe to run multiple times.
...
print(f"\n{'=' * 55}")
print(f"  {B}R&D SAMPLE LOG â€” MENU MIGRATION{E}")
print(f"{'=' * 55}")
...
with app.app_context():
    ...
    # â”€â”€ STEP 1: R&D parent must exist â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ STEP 2: Create rd_sample_log if missing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    if existing:
        ...
        if changed:
            db.session.commit()
            ok("'R&D Sample Log' module updated âœ“")
        else:
            ok("'R&D Sample Log' module already present âœ“")
        mod = existing
    else:
        mod = Module(
            name       = 'rd_sample_log',
        ...
        db.session.add(mod)
        db.session.commit()
        ok(f"'R&D Sample Log' module created (id={mod.id}) âœ“")

    # â”€â”€ STEP 3: Seed default role permissions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ROLE_MATRIX = {
    ...
    print(f"\n{'=' * 55}")
    print(f"  {G}{B}âœ… DONE â€” restart the server to see the menu.{E}")
    print(f"{'=' * 55}\n")
    print("  R&D menu now:")
    print("    ðŸ§ª R&D")
    print("      â”œâ”€â”€ â—ˆ R&D Dashboard")
    print("      â”œâ”€â”€ â¬¡ R&D Projects")
    print("      â”œâ”€â”€ ðŸ“¦ Sample Ready")
    print("      â”œâ”€â”€ ðŸ“‹ Sample History")
    print(f"      â””â”€â”€ ðŸ§ª {B}R&D Sample Log  â† NEW{E}\n")
