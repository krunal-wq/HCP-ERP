...
automatically â€” no need to hardcode credentials here.
...
def run():
    ...
    with app.app_context():
        ...
        print(f"ðŸ“¡ Connected to: {url_str}")
        print(f"ðŸ—‚  Dialect    : {dialect}")

        with engine.begin() as conn:
            if not _table_exists(conn, "office_dispatch_items", dialect):
                print("âŒ Table 'office_dispatch_items' not found in this database.")
                print("   â†’ Start the Flask app once so SQLAlchemy creates all tables, "
            ...
            for name, ddl in col_defs:
                if _column_exists(conn, "office_dispatch_items", name, dialect):
                    print(f"âœ“ Column already exists: {name}")
                    continue
                conn.execute(text(ddl))
                print(f"âœ… Added column: {name}")
                added += 1

            # Defensive backfill â€” any legacy NULL rows get 'pending'
            conn.execute(text(
            ...
            if not _table_exists(conn, "sample_approval_logs", dialect):
                if dialect.startswith("mysql"):
                    ...
                    """))
                print("âœ… Created table: sample_approval_logs")
                added += 1
            else:
                print("âœ“ Table already exists: sample_approval_logs")

            # â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            # Seed NPDStatus rows for 'sent_to_client' and 'rejected'
            # (only if they don't exist â€” preserves user-defined color/icon)
            # â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            if _table_exists(conn, "npd_statuses", dialect):
                seed_rows = [
                    # (slug, name, color, icon, sort_order)
                    ('sent_to_client', 'Sent to Client', '#0ea5e9', 'ðŸ“¤', 70),
                    ('rejected',       'Rejected',       '#dc2626', 'âŒ', 90),
                ]
                ...
                for slug, name, color, icon, sort_order in seed_rows:
                    ...
                    if exists:
                        print(f"âœ“ NPDStatus already exists: {slug}")
                        continue
                    conn.execute(text("""
                    ...
                    print(f"âœ… Seeded NPDStatus: {slug} ({name})")
                    added += 1
            else:
                print("âš ï¸  Table 'npd_statuses' not found â€” skipping status seed.")

        if added == 0:
            print("ðŸŽ‰ Nothing to do â€” schema is already up to date.")
        else:
            print(f"ðŸŽ‰ Migration complete â€” {added} column(s) added.")
