...
table. Yeh column form mein "Previous Salary (â‚¹/month)" field ke through
...
Safe to re-run â€” INFORMATION_SCHEMA check karta hai pehle ki column
...
def migrate():
    ...
    with app.app_context():
        # Sanity check â€” kya `employees` table exists?
        try:
            db.session.execute(db.text("SELECT 1 FROM employees LIMIT 1"))
        except Exception as e:
            print(f"âŒ 'employees' table query failed: {e}")
        ...
        if failed:
            print("\nâš   Column add nahi ho paaya â€” error message check karo.")
            sys.exit(2)
        else:
            print("\nâœ“ Migration complete. Ab Flask service restart karo:")
