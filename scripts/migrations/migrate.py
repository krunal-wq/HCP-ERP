...
â•‘           ERPDEMO â€” MASTER MIGRATION SCRIPT                      â•‘
â•‘                                                                  â•‘
â•‘  Pehli baar: python setup.py  (packages + DB create)            â•‘
â•‘  Phir:       python migrate.py (tables + columns + data)         â•‘
â•‘                                                                  â•‘
â•‘  âœ… Safe to run MULTIPLE TIMES                                    â•‘
â•‘  âœ… Already existing tables/columns â†’ SKIP                       â•‘
â•‘  âœ… Already existing data â†’ SKIP                                 â•‘
â•‘  âœ… Missing cheez â†’ CREATE / INSERT                              â•‘
...
print(f"\n{'='*60}")
print(f"  {B}ERPDEMO â€” MASTER MIGRATION{E}")
if ARGS.only_sync:
...
# AUTO SCHEMA SYNC â€” helper (used by STEP FINAL below)
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
def _run_auto_schema_sync(db, raw, cur, args):
    ...
    if missing_fks:
        ...
        for f in missing_fks:
            print(f"    + {f['table']}.{f['column']} â†’ {f['ref_table']}.{f['ref_column']}  (ON DELETE {f['on_delete']})")

    if not has_changes:
        ok("Schema is already in sync with models â€” nothing to do.")
        return

    # â”€â”€ GENERATE mode: write standalone file and return â”€â”€
    if args.generate:
        _write_standalone_migration(diff, args.output)
        return

    # â”€â”€ DRY-RUN mode: stop here â”€â”€
    if args.dry_run:
        warn("\nDry-run mode â€” no changes applied. Re-run without --dry-run to apply.")
        return

    # â”€â”€ APPLY mode (default) â”€â”€
    ...
    # 2. Missing columns â€” re-check at apply time
    for c in missing_columns:
        cur.execute(
    ...
    for f in missing_fks:
        ...
        try:
            ...
            raw.commit()
            ok(f"added FK `{f['table']}.{f['column']}` â†’ `{f['ref_table']}.{f['ref_column']}`")
            stats['fks'] += 1
        except Exception as e:
...
def _write_standalone_migration(diff, output_path):
    ...
    if diff['missing_tables']:
        lines.append("    # â”€â”€ Missing tables â”€â”€")
    ...
    if diff['missing_columns']:
        lines.append("    # â”€â”€ Missing columns â”€â”€")
    ...
    if diff['missing_indexes']:
        lines.append("    # â”€â”€ Missing indexes â”€â”€")
    ...
    if diff['missing_fks']:
        lines.append("    # â”€â”€ Missing foreign keys â”€â”€")
    ...
    body = '\n'.join(lines) if lines else "    info('Schema is already in sync â€” nothing to do.')"
...
Auto-generated migration â€” safe to run multiple times.
Generated: {timestamp}
"""
import sys, os, re
...
with app.app_context():
    ...
    print("\\n" + C + "MIGRATION â€” Auto-generated schema sync" + E)
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# DATA MERGE â€” copy rows from a mysqldump SQL file into the live DB,
...
#   â€¢ Plain import will FAIL on duplicate-key errors (already-imported
#     IDs) and abort half-way through.
#   â€¢ `INSERT IGNORE` would skip silently but corrupt FK relationships
#     when sequences differ between source and target.
#   â€¢ `mysqldump --insert-ignore` rewrites the file but still drops
...
def _run_data_merge(raw, cur, sql_file_path, tables_filter=None):
    ...
    # â”€â”€ Read whole file (mysqldump files are usually <500MB; if yours is
    ...
    # â”€â”€ Parse INSERT statements
    # mysqldump emits one of two forms:
    #   INSERT INTO `table` VALUES (..),(..),(..);
    #   INSERT INTO `table` (`col1`,`col2`) VALUES (..),(..);
    # We handle both. The VALUES tuples are kept as raw SQL strings â€” we
    ...
    # â”€â”€ First pass: discover which tables exist in the dump
    tables_in_dump = []
    ...
    if not tables_in_dump:
        err("Koi INSERT statement file mein nahi mila â€” kya yeh mysqldump file hai?")
        return False
    ...
    # â”€â”€ Disable FK checks for the whole merge (re-enable at end)
    ...
    # â”€â”€ For each table, find every INSERT block and merge row-by-row
    for tname in tables_in_dump:
        ...
        if cur.fetchone()[0] == 0:
            warn(f"  {tname}: live DB mein table nahi hai â€” skip")
            continue
        ...
        if not pk_cols:
            # No primary key â€” fall back to "skip the whole table if any
            # rows are already present" (safer than blind insert that
            # could create duplicates).
            cur.execute(f"SELECT COUNT(*) FROM `{tname}`")
            if cur.fetchone()[0] > 0:
                warn(f"  {tname}: PK nahi hai aur live DB mein data hai â€” skip")
                continue
        ...
        for stmt_match in table_insert_re.finditer(sql_text):
            # Determine columns for this INSERT â€” explicit list or default
            ...
            for tup in tuple_strings:
                ...
                # â”€â”€ Build the duplicate-check query
                ...
                # â”€â”€ INSERT this row (re-emitting the original VALUES
                # tuple verbatim â€” preserves NULL, hex literals, etc.)
                ...
                try:
                    ...
                    # though the PK was different â€” treat as skip
                    msg = str(e).lower()
    ...
    print()
    ok(f"DATA MERGE done â€” total inserted={grand_inserted}, skipped={grand_skipped}, errors={grand_errors}")
    return True
...
with app.app_context():
    ...
    # EARLY EXIT â€” --only-sync mode skips all classic steps
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    if ARGS.only_sync:
        ...
        print(f"\n{'='*60}")
        print(f"  {G}{B}âœ… SYNC COMPLETE!{E}")
        print(f"{'='*60}\n")
        sys.exit(0)

    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # EARLY EXIT â€” --data-only runs ONLY the data merge
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    if ARGS.data_only:
        ...
        print(f"\n{'='*60}")
        print(f"  {G}{B}âœ… DATA MERGE COMPLETE!{E}")
        print(f"{'='*60}\n")
        sys.exit(0)

    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 1 â€” Create all tables
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    step("STEP 1: Tables create kar raha hai...")
    db.create_all()
    ok("db.create_all() done â€” sari tables ready")

    # â”€â”€ Office Dispatch Tables (Sample Ready â†’ Send to Office) â”€â”€
    try:
    ...
    # â”€â”€ Add new columns to office_dispatch_items if missing â”€â”€
    try:
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 1B â€” Critical columns jo Step 7 se PEHLE chahiye
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    for _tbl, _col, _defn in _critical_cols:
        try:
            ...
            if not _cur1b.fetchone():
                ...
                _con1b.commit()
                ok(f"{_tbl}.{_col} column add kiya âœ“")
            else:
                ok(f"{_tbl}.{_col} already exists âœ“")
        except Exception as _e:
    ...
    try:
        ...
        if not _cur1b.fetchone():
            ...
            _con1b.commit()
            ok("employees.employee_id unique index added âœ“")
        else:
            ok("employees.employee_id unique index already exists âœ“")
    except Exception as _e:
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 2 â€” Fix column types (MEDIUMTEXT for large data)
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 3 â€” Missing columns: employees
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    step("STEP 3: employees table â€” missing columns add kar raha hai...")
    emp_cols = [
        # Basic
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 4 â€” Missing columns: leads
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    step("STEP 4: leads table â€” missing columns add kar raha hai...")
    lead_cols = [
        ('code',              'VARCHAR(20)'),
    ...
    ok("leads.title â†’ NULL allowed (import fix)")

    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 5 â€” Missing columns: users & other tables
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    step("STEP 5: Other tables â€” missing columns...")
    other = [
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 5F â€” Contractor Document Fields
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    step("STEP 5F: contractors table â€” document fields add kar raha hai...")
    contractor_doc_cols = [
        # Document Numbers
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 5B â€” Soft Delete columns (is_deleted + deleted_at)
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    for tbl, idx_name, col in soft_indexes:
        if table_exists(tbl) and col_exists(tbl, col):
            try:
                ...
                pass  # already exists â€” skip silently
    ok("Soft delete indexes ready")

    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 5E â€” created_at / created_by / updated_at / updated_by
    ...
    # Seed default values (INSERT IGNORE â€” won't overwrite existing)
    salary_defaults = [
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 5D â€” Salary Components table + default components
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    # Seed default components (system components â€” is_system=1)
    ...
    if not User.query.filter_by(username='admin').first():
        ...
        db.session.commit()
        ok("Admin user created â†’ admin / HCP@123")
    else:
        ok("Admin user already exists")

    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 7 â€” Seed: Modules & Permissions
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    modules_data = [
        ...
        ("crm",               "CRM",            "ðŸ“Š", "/crm",                   2),
        ("crm_leads",         "Leads",          "ðŸ“‹", "/crm/leads",             3),
        ("crm_clients",       "Clients",        "ðŸ‘¥", "/crm/clients",           4),
        ("hr",                "HR",             "ðŸ‘”", "/hr",                    5),
        ("hr_employees",      "Employees",      "ðŸªª", "/hr/employees",          6),
        ("hr_contractors",    "Contractors",    "ðŸ¤", "/hr/contractors",        7),
        ("npd",               "NPD",            "ðŸ”¬", "/npd",                   8),
        ("rd",                "R&D",            "ðŸ§ª", "/rd",                    9),
        ("rd_projects",       "R&D Projects",   "ðŸ—‚ï¸", "/rd/projects",           10),
        ("rd_sample_ready",   "Sample Ready",   "ðŸ“¦", "/rd/sample-ready",       11),
        ("rd_sample_history", "Sample History", "ðŸ“‹", "/rd/sample-history",     12),
        ("masters",           "Masters",        "âš™ï¸", "/masters",               13),
        ("approvals",         "Approvals",      "âœ…", "/approvals",             14),
        ("user_mgmt",         "Users",          "ðŸ‘¤", "/admin/users",           15),
        ("audit_logs",        "Audit Logs",     "ðŸ”", "/admin/audit-logs",      16),
    ]
    mod_added = 0
    ...
    # â”€â”€ can_import column ensure karo BEFORE RolePermission query â”€â”€
    try:
        import pymysql
        ...
        for _fix_tbl in ('role_permissions', 'user_permissions'):
            ...
            if not _rc_cur.fetchone():
                ...
                _rc.commit()
                ok(f"{_fix_tbl}.can_import column add kiya âœ“")
            else:
                ok(f"{_fix_tbl}.can_import already exists âœ“")
        _rc_cur.close()
        _rc.close()
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 8 â€” Seed: Lead Statuses, Sources, Categories, Ranges
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    statuses = [
        ...
        ("close",            "âœ…", "#059669", 3),
        ("cancel",           "âŒ", "#dc2626", 4),
        ("NPD Project",      "ðŸ§ª", "#8b5cf6", 5),
        ("Existing Project", "ðŸ“¦", "#0ea5e9", 6),
    ]
    s_added = 0
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 8B â€” Sample Orders table
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 8A â€” Quotations table
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 7B â€” Category Master, UOM Master, HSN Code Master
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    # customer_id â†’ lead_id migrate karo (agar purana data hai)
    if col_exists('quotations', 'customer_id') and col_exists('quotations', 'lead_id'):
        try:
            ...
            raw.commit()
            ok("quotations: customer_id â†’ lead_id data migrated")
        except: pass
    ...
    # â”€â”€ Sample Orders soft delete columns â”€â”€
    so_soft = [
        ('is_deleted', 'TINYINT(1) NOT NULL DEFAULT 0'),
        ('deleted_at', 'DATETIME NULL DEFAULT NULL'),
        ('deleted_by', 'INT NULL DEFAULT NULL'),
    ...
    # invoice_file column â€” purani table ke liye (safe to run multiple times)
    safe_add('sample_orders', 'invoice_file', 'VARCHAR(300)')

    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 8B2 â€” client_addresses: brand_index column
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    step("STEP 8B2: client_addresses â€” brand_index column add kar raha hai...")
    safe_add('client_addresses', 'brand_index', 'INT DEFAULT 0 AFTER `client_id`')
    ok("client_addresses.brand_index ready")

    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 8B3 â€” client_masters: client_type column drop
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    step("STEP 8B3: client_masters â€” client_type column drop kar raha hai...")
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 8B4 â€” leads: code column drop
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    step("STEP 8B4: leads â€” code column drop kar raha hai...")
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 8C â€” Email Templates table
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 8D â€” Lead Contributions table
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 8E â€” Contribution Config table
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    # â”€â”€ NPD Project R&D param defaults â”€â”€
    safe_add('npd_projects', 'rd_param_defaults', 'TEXT')

    # â”€â”€ Ensure npd_comments has attachment column â”€â”€
    safe_add('npd_comments', 'attachment', 'VARCHAR(300)')

    # â”€â”€ NPD Comments & Notes tables â”€â”€
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 11C â€” NPD Statuses table create + seed
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    if ns_count == 0:
        npd_status_defaults = [
            ...
            # â”€â”€ Project Statuses â”€â”€
            ('Not Started',                         'not_started',        '#6b7280', 'â­•', 1),
            ('Sample Inprocess',                    'sample_inprocess',   '#8b5cf6', 'ðŸ”„', 2),
            ('Sample Send to Client',               'sample_sent',        '#06b6d4', 'ðŸ“¤', 3),
            ('On Hold',                             'on_hold',            '#f59e0b', 'â¸ï¸', 4),
            ('Sample Rejected',                     'sample_rejected',    '#ef4444', 'âŒ', 5),
            ('Sample Approved',                     'sample_approved',    '#10b981', 'âœ…', 6),
            ('Cancelled',                           'cancelled',          '#dc2626', 'ðŸš«', 7),
            ('Finish',                              'finish',             '#22c55e', 'ðŸ', 8),
            ('Sample Ready',                        'sample_ready',       '#3b82f6', 'ðŸ“¦', 9),
            ('Sent to Office',                      'sent_to_office',     '#6366f1', 'ðŸ¢', 10),
            ('Rejected by Office',                  'rejected_by_office', '#f97316', 'ðŸ”´', 11),
            ('Assign',                              'assign',             '#64748b', 'ðŸ‘¤', 12),
            ('Assigned',                            'assigned',           '#7c3aed', 'âœ”ï¸', 13),
            # â”€â”€ Milestone Names â”€â”€
            ('BOM',                                 'bom',                '#7c3aed', 'ðŸ“„', 14),
            ('Ingredients List & Marketing Sheet',  'ingredients',        '#2563eb', 'ðŸ“‹', 15),
            ('Quotation',                           'quotation',          '#d97706', 'ðŸ’°', 16),
            ('Packing Material',                    'packing_material',   '#059669', 'ðŸ“¦', 17),
            ('Artwork / Design',                    'artwork',            '#db2777', 'ðŸŽ¨', 18),
            ('Artwork QC Approval',                 'artwork_qc',         '#10b981', 'âœ…', 19),
            ('FDA',                                 'fda',                '#1d4ed8', 'ðŸ›ï¸', 20),
            ('Barcode',                             'barcode',            '#374151', 'ðŸ”¢', 21),
        ]
        ...
        # Already seeded â€” missing milestone names add karo
        milestone_names = [
            ('BOM',                                 'bom',                '#7c3aed', 'ðŸ“„', 14),
            ('Ingredients List & Marketing Sheet',  'ingredients',        '#2563eb', 'ðŸ“‹', 15),
            ('Quotation',                           'quotation',          '#d97706', 'ðŸ’°', 16),
        ...
        for name, slug, color, icon, sort in milestone_names:
            ...
            if cur.fetchone()[0] == 0:
                ...
                added += 1
        raw.commit()
        ok(f"npd_statuses: {ns_count} existing + {added} milestone names added") if added else ok(f"npd_statuses: {ns_count} records already exist â€” skip")

    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 11B â€” NPD Project: missing columns add karo
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    step("STEP 11B: npd_projects â€” missing columns add kar raha hai...")
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 12 â€” R&D Test Parameter Master table + seed
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    if param_count == 0:
        ...
        ok(f"rd_test_parameters: {param_count} parameters already exist â€” seed skip")
    ...
    for sort_order, name, unit, is_active in rows:
        ...
        print(f"     [{sort_order}] {name:<20} unit={unit or 'â€”':<8} {status}")


    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 13 â€” Seed: NPD Milestone Templates (Default)
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    step("STEP 13: NPD Milestone Templates seed kar raha hai...")

    if not table_exists('npd_milestone_templates'):
        ok("npd_milestone_templates table exist nahi â€” skip (pehle migrate chalao)")
    else:
        ...
        new_milestones = [
            ...
            ('bom',                 'BOM',                                'ðŸ“„', 'both', 1, 0, 1),
            ('ingredients',         'Ingredients List & Marketing Sheet', 'ðŸ“‹', 'both', 1, 0, 2),
            ('quotation',           'Quotation',                          'ðŸ’°', 'both', 1, 0, 3),
            ('packing_material',    'Packing Material',                   'ðŸ“¦', 'both', 1, 0, 4),
            ('artwork',             'Artwork / Design',                   'ðŸŽ¨', 'both', 1, 0, 5),
            ('artwork_qc',          'Artwork QC Approval',                'âœ…', 'both', 1, 0, 6),
            ('fda',                 'FDA',                                'ðŸ›ï¸', 'both', 1, 0, 7),
            ('barcode',             'Barcode',                            'ðŸ”¢', 'both', 1, 0, 8),
        ]
        seeded = 0
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 13B â€” Milestone Status Master table + seed
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    if ms_st_count == 0:
        milestone_status_defaults = [
            # (name,                                slug,                 color,     icon, sort)
            # â”€â”€ General statuses â”€â”€
            ('Pending',                             'pending',            '#6b7280', 'â³', 1),
            ('In Progress',                         'in_progress',        '#f59e0b', 'ðŸ”„', 2),
            ('Approved',                            'approved',           '#10b981', 'âœ…', 3),
            ('Rejected',                            'rejected',           '#ef4444', 'âŒ', 4),
            ('Skipped',                             'skipped',            '#94a3b8', 'â­ï¸', 5),
            # â”€â”€ Milestone names â”€â”€
            ('BOM',                                 'bom',                '#7c3aed', 'ðŸ“„', 6),
            ('Ingredients List & Marketing Sheet',  'ingredients',        '#2563eb', 'ðŸ“‹', 7),
            ('Quotation',                           'quotation',          '#d97706', 'ðŸ’°', 8),
            ('Packing Material',                    'packing_material',   '#059669', 'ðŸ“¦', 9),
            ('Artwork / Design',                    'artwork',            '#db2777', 'ðŸŽ¨', 10),
            ('Artwork QC Approval',                 'artwork_qc',         '#10b981', 'âœ…', 11),
            ('FDA',                                 'fda',                '#1d4ed8', 'ðŸ›ï¸', 12),
            ('Barcode',                             'barcode',            '#374151', 'ðŸ”¢', 13),
        ]
        ...
        # Already has some â€” just add missing milestone names
        ...
        for idx, (mtype, title, icon) in enumerate(templates, 20):
            ...
            if cur.fetchone()[0] == 0:
                ...
                added += 1
        raw.commit()
        ok(f"milestone_statuses: {ms_st_count} records exist, {added} milestone names added") if added else ok(f"milestone_statuses: {ms_st_count} records already exist â€” skip")

    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 14 â€” User-wise Permission Table
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 14B â€” Remove deprecated NPD columns
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    def safe_drop(table, col):
        ...
        if table_exists(table) and col_exists(table, col):
            try:
                ...
                raw.commit()
                ok(f"{table}.{col} dropped âœ“")
                return True
            except Exception as e:
                warn(f"Could not drop {table}.{col}: {e}")
        else:
            warn(f"{table}.{col} â€” already removed or doesn't exist, skip")
        return False
    ...
    for old_name, new_name in RENAME_MAP.items():
        ...
        if not old_mod:
            continue
        if new_mod:
            # Merge: move permissions from old â†’ new, delete old
            ...
            ok(f"Duplicate '{old_name}' â†’ '{new_name}' merged & deleted")
        else:
            old_mod.name = new_name
            ok(f"Module '{old_name}' â†’ '{new_name}' renamed")
    ...
    if _rd_trials_mod:
        ...
        db.session.commit()
        ok("rd_trials module removed from sidebar âœ“")
    for child_name, parent_name in parent_map.items():
        ...
        if child and parent and child.parent_id != parent.id:
            ...
            ok(f"'{child_name}' parent fixed â†’ '{parent_name}'")
    ...
    if not att_mod:
        att_mod = Module(
            ...
            icon='ðŸ•', url_prefix='/hr/attendance',
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 17 â€” Comprehensive HR Rules Tables
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    # â”€â”€ Early Coming Rules table â”€â”€
    ...
    _cur17.close()
    _c17.close()
    ok("âœ… All HR Rules tables ready!")

    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # PACKING MATERIAL â€” new columns (fresh connection)
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    try:
        ...
        for _col, _defn in [
            ...
            if _pm_cur.fetchone()[0] == 0:
                ...
                _pm_con.commit()
                ok(f"npd_packing_materials.{_col} column add kiya âœ“")
            else:
                ok(f"npd_packing_materials.{_col} already exists âœ“")
        _pm_cur.close()
        _pm_con.close()
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 18 â€” Packing Entries Table (Sample Receipt Log)
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    # â”€â”€ Safe add missing columns (re-run safe) â”€â”€
    _pk_cols = [
        ('batch_no',           "VARCHAR(50)   DEFAULT ''"),
        ('mfg_date',           "VARCHAR(20)   DEFAULT ''"),
        ('exp_date',           "VARCHAR(20)   DEFAULT ''"),
    ...
    # â”€â”€ Indexes (safe â€” skip if already exist) â”€â”€
    for _iname, _icol in [
    ...
    _cur18.close()
    _c18.close()
    ok("âœ… packing_entries setup complete!")


    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 19 â€” Item Master (material_types, material_groups, materials)
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    # â”€â”€ Seed default Material Types â”€â”€
    _default_types = [
    ...
    # â”€â”€ Add 'material' module to permissions system â”€â”€
    try:
    ...
    _cur19.close()
    _c19.close()
    ok("âœ… Item Master tables setup complete!")


    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 20 â€” RD Project Logs table
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP 21 â€” RD Sub Assignments table
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    #
    # CLI flags:
    #   (default)        â†’ apply missing changes
    #   --dry-run        â†’ show what would change, do not apply
    #   --generate       â†’ write a standalone migrate_diff.py
    #   --json           â†’ print machine-readable diff (and exit)
    #   --skip-sync      â†’ skip this step entirely
    #   --only-sync      â†’ skip all previous steps, run only this
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    if not ARGS.skip_sync:
        _run_auto_schema_sync(db, raw, cur, ARGS)

    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # STEP DATA-MERGE â€” Smart data import from a mysqldump file
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    print(f"\n{'='*60}")
    print(f"  {G}{B}âœ… MIGRATION COMPLETE!{E}")
    print(f"{'='*60}")
