...
# -------------------------------------------------------------------------------------
"""
clean_for_testing.py â€” Sirf Admin + Master data rakho, baaki sab clean karo
...
print(f"\n{'='*62}")
print(f"  {B}CLEAN FOR TESTING â€” DATA RESET SCRIPT{E}")
print(f"{'='*62}")
...
def clean(table, where=None, params=None):
    if not tbl_exists(table):
        warn(f"{table} â€” nahi mila, skip"); return
    try:
        if where:
        ...
        con.commit()
        ok(f"{table} â€” {cur.rowcount} rows deleted")
    except Exception as e:
        con.rollback(); err(f"{table} â€” {e}")

# STEP 1: Users â€” sirf admin/master rakhenge
...
if delete_ids:
    ...
    ok("Sirf admin users hain â€” kuch delete nahi kiya")
