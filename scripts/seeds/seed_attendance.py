...
# -------------------------------------------------------------------------------------
"""
seed_attendance.py â€” Dummy employees + attendance data seed karo
...
def seed():
    with app.app_context():
        print("\nðŸŒ± Seeding dummy data...\n")

        # â”€â”€ Step 1: Employees â”€â”€
        created_emps = []
        for emp_code, emp_id, fname, lname, dept, desig, etype in EMPLOYEES:
            ...
            if existing:
                print(f"   â­  Employee {emp_code} already exists")
            ...
            print(f"   âœ… Employee {emp_code} â€” {fname} {lname} ({dept})")

        db.session.commit()
        print(f"\n   ðŸ‘¥ {len(created_emps)} employees ready\n")

        # â”€â”€ Step 2: Attendance â€” last 30 days â”€â”€
        today      = date.today()
        ...
        if existing_att > 200:
            ...
            if today_count > 0:
                print(f"   â­  Attendance already seeded ({existing_att} records, today: {today_count})")
                print("\nâœ… Seed complete!\n")
                return
            print(f"   â„¹ï¸  Past data exists but today missing â€” adding today's data...")
        ...
        for emp_code, emp_id, fname, lname, dept, desig, etype in EMPLOYEES:
            ...
            while current <= today:
                ...
                if rand < 0.05:
                    ...
                    # PRESENT â€” normal (65%)
                    in_min  = random.randint(0, 14)   # 9:00â€“9:14
        ...
        db.session.commit()
        print(f"   âœ… {att_count} attendance records added")
        print(f"   âœ… {punch_count} raw punch logs added")
        print(f"\nâœ… Seed complete! Ab /hr/attendance visit karo.\n")
