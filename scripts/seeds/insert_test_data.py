...
# -------------------------------------------------------------------------------------
"""
NPD Sample Ready â€” Test Data Script
====================================
...
with app.app_context():
    ...
    if not admin:
        print("âŒ Koi user nahi mila DB mein. Pehle migrate.py run karo.")
        sys.exit(1)
    ...
    for td in TEST_PROJECTS:
        ...
        if existing:
            ...
            db.session.commit()
            print(f"  â†º  {td['code']} already exists â€” status â†’ sample_ready")
            skipped += 1
            continue
        ...
        db.session.add(proj)
        db.session.commit()
        print(f"  âœ… Created: {td['code']} â€” {td['product_name']} ({td['client_name']})")
        created += 1

    print(f"\n{'='*50}")
    print(f"  Done! Created: {created}  |  Updated: {skipped}")
    print(f"  Ab /npd/sample-ready kholo â€” 5 projects dikhenge")
    print(f"{'='*50}\n")
