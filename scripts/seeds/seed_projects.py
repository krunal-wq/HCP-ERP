...
# -------------------------------------------------------------------------------------
"""
seed_projects.py â€” Dummy NPD & EPD Projects seed karo
...
try:
    ...
    print(f"  {R}âŒ App load failed: {e}{E}")
    sys.exit(1)
...
with app.app_context():
    ...
    for d in npd_data:
        if NPDProject.query.filter_by(code=d['code']).first():
            warn(f"{d['code']} already exists â€” skip")
            continue
        proj = NPDProject(
            code=d['code'],
    ...
    print(f"\n{'='*55}")
    print(f"  {G}{B}âœ… DONE! {npd_added} NPD + {epd_added} EPD projects created!{E}")
    print(f"{'='*55}")
