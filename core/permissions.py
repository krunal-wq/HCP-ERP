"""
permissions.py â€” Permission helpers used across the app

Permission model: USER-ONLY (no role fallback).
  - Admin role â†’ default full rights (with safety: can_view kabhi False nahi).
  - Non-admin â†’ access sirf UserPermission record se milega.
  - Koi UserPermission record nahi â†’ no access (menu bhi hide).
...
# â”€â”€ Sub-permission keys per module â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Ye dict define karta hai har module ke andar kaunse granular sub-permissions hain
MODULE_SUB_PERMS = {
    # â”€â”€ CRM parent module â€” sub-perms are the "Settings" page chips
    ...
    'hr_employees': [
        ('professional_tab', 'Professional Tab'),
        ('personal_kyc_tab', 'Personal & KYC Tab'),
        ('bank_tab',         'Bank Detail Tab'),
        ('salary_tab',       'Salary Tab'),
        ('education_tab',    'Education Tab'),
    ...
    'hr': [
        ...
        ('holiday',           'Holiday'),
        # â”€â”€ Attendance Log views (added) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ('att_sync',          'Attendance: Sync Data'),
        ('att_daily',         'Attendance: Daily View'),
        ('att_monthly',       'Attendance: Monthly View'),
        # Leave menus
        ('leave_apply',       'Leave: Apply'),
        ('leave_list',        'Leave: Applications'),
        ('leave_balance',     'Leave: Balance'),
        # Rules & policy menus
        ('shift_master',      'Shift Master'),
        ('location_master',   'Location Master'),
        ('late_coming_rules', 'Late Coming Rules'),
        ('early_going_rules', 'Early Going Rules'),
        ('overtime_rules',    'Overtime Rules'),
        ('leave_policy',      'Leave Policy'),
        ('lop_rule',          'LOP Rule'),
        ('absent_rules',      'Absent Rules'),
        ('comp_off_rules',    'Comp Off Rules'),
        # HR masters menus
    ...
    'npd_projects': [
        ...
        ('notes',                 'Notes'),
        ('milestone',             'Milestone'),
        # â”€â”€ Per-milestone visibility (sidebar items in Milestones tab) â”€â”€
        # Milestone tab khud `milestone` perm se gate hoti hai. Yeh
        # granular keys decide karte hain ki sidebar me kaunsa milestone
        # item dikhe. Backend data structure (ms_1, ms_2, â€¦) untouched
        # rehta hai â€” sirf display level pe filter hota hai.
...
# â”€â”€ Fetch permission for current user â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def get_perm(module_name):
    """
    Permission model (user-only, no role fallback):
    1. Admin role â†’ always _full_perm() (default full rights, apart from explicit
       UserPermission override â€” and admin ka can_view kabhi False nahi hota)
    2. Non-admin with UserPermission record â†’ use that record
    3. Non-admin with no UserPermission â†’ _no_perm()  (nothing until explicitly granted)
    """
    ...
    try:
        ...
        if is_admin:
            # Admin ke liye UserPermission optional â€” agar hai to respect karo,
        ...
        # Non-admin: sirf UserPermission se chalega. Koi record nahi â†’ no access.
...
def get_sub_perm(module_name, key):
    ...
    Admin â†’ always True.
    Non-admin â†’ UserPermission.sub_permissions se dekhega; record nahi â†’ False.
    """
    if not current_user.is_authenticated:
        return False
    ...
    try:
        ...
        if user_perm is not None:
            ...
            # Missing key â†’ False (explicit grant chahiye)
            return subs.get(key, False)
        # UserPermission record nahi â†’ no access
        return False
    except Exception:
        return False
...
# â”€â”€ Get user's grid columns for a module â”€â”€
...
def get_module_active(module_name):
    ...
    Admin â†’ always True. Non-admin â†’ UserPermission.can_view hi dekhega.
    """
    if not current_user.is_authenticated:
        return False
    ...
    try:
        ...
        # Koi record nahi â†’ no access
        return False
    except Exception:
        return False


# â”€â”€ Get all visible menu modules for current user â”€â”€
def get_menu_modules():
    """
    Returns top-level modules visible to current user in the sidebar.
    Admin â†’ all active modules (except ones explicitly disabled via UserPermission).
    Non-admin â†’ only modules with UserPermission.can_view = True.
    """
    if not current_user.is_authenticated:
        return []
    ...
    if current_user.role == 'admin':
        # Admin: check UserPermission override; if no override â†’ full access
        visible = []
...
def get_visible_sub_modules(parent_module):
    ...
    Admin â†’ all active children (minus explicit disables).
    Non-admin â†’ only children with UserPermission.can_view = True.
    """
    if not current_user.is_authenticated:
        return []
...
# â”€â”€ Seed default modules and permissions â”€â”€
DEFAULT_MODULES = [
    {'name':'dashboard',    'label':'Dashboard',      'icon':'ðŸ ', 'url_prefix':'/',          'sort_order':1},
    {'name':'crm',          'label':'CRM',            'icon':'ðŸ“Š', 'url_prefix':'/crm',       'sort_order':2},
    {'name':'crm_leads',    'label':'Leads',          'icon':'ðŸ“‹', 'url_prefix':'/crm/leads', 'sort_order':3,  'parent':'crm'},
    {'name':'crm_quotations','label':'Quotations','icon':'ðŸ“„','url_prefix':'/crm/quotations','sort_order':6,'parent':'crm'},
    {'name':'crm_sample_orders','label':'Sample Orders','icon':'ðŸ§¾','url_prefix':'/crm/sample-orders','sort_order':5,'parent':'crm'},
    ...
    {'name':'masters',      'label':'Masters',        'icon':'âš™ï¸', 'url_prefix':'/masters',   'sort_order':8},
    {'name':'user_mgmt',    'label':'User Management','icon':'ðŸ”‘', 'url_prefix':'/admin/users','sort_order':9},
]
