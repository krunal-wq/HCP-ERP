"""
audit_helper.py â€” Central audit logging with full JSON snapshots
"""
import json
    ...
    if action == 'DELETE' and obj is not None:
        ...
        if old_dict and new_dict:
            ...
            if not detail and changes:
                detail = ', '.join(f"{k}: {v['before']} â†’ {v['after']}" for k, v in list(changes.items())[:5])
        if obj is not None:
