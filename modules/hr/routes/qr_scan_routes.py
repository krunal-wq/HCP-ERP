"""
qr_scan_routes.py â€” v2.1 â€” DIAGNOSTIC VERSION
QR Code Scanner kiosk: punch register + employee display

Kya karta hai:
  â€¢ External USB/Bluetooth scanner ya mobile/system camera se QR scan
  â€¢ Code milte hi:
  ...
  â€¢ Cooldown: same employee 30 sec mein dobara scan kare to ignore
  â€¢ Auto IN/OUT detect: pehla scan = IN, doosra scan = OUT, aage ke
    scans punch_out ko latest update karte hain

Login NAHI chahiye â€” kiosk mode (entrance gate ke liye).

Routes:
  GET  /hr/attendance/qr-scan         â† kiosk page render
  POST /hr/attendance/qr-lookup       â† scan API: punch + return data
  GET  /hr/attendance/qr-version      â† debug: which version is running
"""
import json
import sys
...
# MODULE-LOAD BANNER â€” server restart pe terminal mein dikhega
# Aap dekh sakte ho ki naya file load hua hai ya purana
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
sys.stderr.write("\n" + "=" * 60 + "\n")
sys.stderr.write("  âœ“  QR-SCANNER v2.3 LOADED  (in/out lists + stats)\n")
...
def _refresh_attendance(emp_code_log_key, today_date):
    ...
    _log(f"refresh_attendance â†’ punches={len(punches)}, "
...
def _do_punch(emp_code_log_key, today_date):
    ...
    if last_raw and last_raw.log_date:
        ...
        if elapsed < PUNCH_COOLDOWN_SEC:
            _log(f"COOLDOWN â€” {emp_code_log_key} ka pichla scan {int(elapsed)}s "
                 f"pehle hua tha, ignored")
            return ('cooldown', last_raw)

    # â”€â”€ Direction (raw punches ke count se determine karo, Attendance se nahi) â”€â”€
    ...
    # â”€â”€ Attendance recompute â”€â”€
    att = _refresh_attendance(emp_code_log_key, today_date)
    if att:
        _log(f"  âœ“ Attendance row: in={att.punch_in} out={att.punch_out} "
             f"hours={att.total_hours} status={att.status}")
    else:
        _log(f"  âš ï¸  Attendance row NOT created")
...
# Useful for verifying ki QR scans â†’ attendance table mein ja rahe ya nahi
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def qr_debug_attendance():
    ...
    return jsonify({
        ...
        'note': 'Agar attendance_rows=0 hai but raw_punch_counts data hai, '
                'toh attendance create nahi ho raha â€” bug hai.',
    }), 200


# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# DEBUG: GET /hr/attendance/qr-version  â†’ confirms which version is running
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def qr_version():
    return jsonify({
        ...
        'message':  'âœ“ NEW backend running â€” punch logic enabled',
    }), 200
...
#   â€¢ Duplicate scans count nahi hote (1 emp = 1 IN, 1 OUT max)
#   â€¢ Biometric device ke punches bhi shaamil hain
#   â€¢ Page refresh pe persistent rehte hain
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
# Return karta hai:
#   in_list  â†’ har employee jo aaj IN hua, with first punch time (1 entry per emp)
#   out_list â†’ har employee jo aaj OUT hua, with latest punch time (1 entry per emp)
...
def qr_today_list():
    ...
    # â”€â”€ Batch-fetch employees taaki N+1 queries na ho â”€â”€
...
def qr_lookup():
    ...
    if not lookup_only:
        try:
            ...
            db.session.commit()
            _log(f"COMMIT OK â€” action={action} action_at={action_at}")
        except Exception as e:
            db.session.rollback()
            _log(f"âŒ PUNCH FAILED: {type(e).__name__}: {e}")
    ...
    # â”€â”€ Updated attendance fetch â”€â”€
    att = Attendance.query.filter_by(
    ...
    return jsonify({
        ...
        'attendance': {
            ...
            'total_hours': att.working_hours_display if att else 'â€”',
