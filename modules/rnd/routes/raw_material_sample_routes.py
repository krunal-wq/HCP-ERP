"""
raw_material_sample_routes.py â€” Raw Material Sample Request module
==================================================================
Blueprint:  raw_material_sample   at  /raw-material-sample

Module flow:
    NPD / R&D    â†’  creates request   (status = sent_to_purchase)
    Purchase     â†’  picks supplier    (status = supplier_finalized)
    Purchase     â†’  places order      (status = order_placed)
    Purchase     â†’  enters tracking   (status = order_dispatched)
    NPD / R&D    â†’  confirms receipt  (status = sample_received)

Access (role-based):
    NPD / R&D users              â†’ can create, can mark received
    Purchase users               â†’ can finalise supplier / place order / dispatch
    Admin / Manager              â†’ full control (see all + edit any stage)
    Everyone with module access  â†’ can view their own requests
...
from core.permissions import get_perm, get_grid_columns, save_grid_columns
...
# Roles that are treated as "Purchase team" â€” they see the Purchase
# action buttons (finalise supplier / place order / dispatch).
PURCHASE_ROLES   = {'purchase', 'purchase_manager', 'purchase_executive'}

# Roles that are treated as "Requester" â€” NPD / R&D side
...
def _is_purchase(user) -> bool:
    """
    Purchase team â€” can take actions (finalise / order / dispatch).
    ...
    if role in REQUESTER_ROLES:
        return False   # explicit requester â€” never treat as Purchase
    # Unknown role â†’ permission-based detection
    try:
...
# explicit requesters keep their existing access â€” protects existing
...
def _can_action(action_key: str, user=None) -> bool:
    ...
    Strict permission model:
        1. Admin â†’ always allowed (bypass).
        2. Explicit sub-permission chip set in ACP â†’ use that value.
        3. Otherwise â†’ DENIED.
...
def _user_action_perms() -> dict:
    """Return {action_key: bool} dict for all 5 actions â€” for the page template."""
    return {k: _can_action(k) for k in ACTION_KEYS}


def _can_delete(user=None) -> bool:
    """Soft-delete permission â€” admin OR can_delete flag on the module."""
    user = user or current_user
    if _is_admin(user):
        return True
    try:
...
def _can_edit(user=None) -> bool:
    """Edit permission â€” admin OR can_edit flag on the module."""
    user = user or current_user
    if _is_admin(user):
        return True
    try:
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#  Grid columns â€” user-customisable display
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
# 'material_name' is required â€” others optional.
RMS_IMPORT_COLUMNS = [
...
def _parse_date(s):
    """Parse a YYYY-MM-DD or DD-MM-YYYY string. Empty â†’ None."""
    if not s:
        return None
    s = str(s).strip()
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#  API â€” list (with filters + summary counts)
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def api_list():
    ...
    # â”€â”€ Summary counts (apply same visibility filter as listing) â”€â”€
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#  API â€” fetch single record + activity log
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def api_get(rid):
    ...
    # Permission gate â€” non-admin / non-purchase can see only their own
    if not (_is_admin(current_user) or _is_purchase(current_user)):
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#  API â€” create new request   (NPD / R&D)
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def api_create():
    ...
    _notify(
        ...
        body=f'{material} â€” requested by '
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#  API â€” edit (only allowed BEFORE supplier is finalised)
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def api_edit(rid):
    ...
    if r.status not in MUTABLE_STATUSES:
        return jsonify({
            'error': 'This request is locked â€” supplier already finalised.'
        }), 409
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#  API â€” Purchase: finalise supplier
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def api_finalize_supplier(rid):
    ...
    _notify(
        ...
        title=f'{r.request_no} â€” supplier finalised',
    ...
    audit('raw_material_sample', 'FINALIZE_SUPPLIER',
          current_user.id, current_user.username,
          f'{r.request_no} â†’ {supplier}', commit=True)

    return jsonify({'ok': True, 'item': r.to_dict()})


# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#  API â€” Purchase: place order
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def api_place_order(rid):
    ...
    _notify(
        r, _requester_followers(r),
        title=f'{r.request_no} â€” order placed',
    ...
    audit('raw_material_sample', 'PLACE_ORDER',
          current_user.id, current_user.username,
          f'{r.request_no} â†’ {order_no}', commit=True)

    return jsonify({'ok': True, 'item': r.to_dict()})


# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#  API â€” Purchase: dispatch / tracking
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def api_dispatch(rid):
    ...
    _log(r, 'STATUS_CHANGE', from_status=prev, to_status='order_dispatched',
         note=f'Dispatched via {courier or "â€”"}, '
              f'tracking: {tracking or "â€”"}')

    # â­ This is the headline notification â€” tell NPD & R&D
    _notify(
        r, _requester_followers(r),
        title=f'{r.request_no} â€” order dispatched âœˆï¸',
        body=f'{r.material_name} dispatched. '
             f'Courier: {courier or "â€”"}, Tracking: {tracking or "â€”"}',
    )

    db.session.commit()
    audit('raw_material_sample', 'DISPATCH',
          current_user.id, current_user.username,
          f'{r.request_no} â†’ {courier} {tracking}', commit=True)

    return jsonify({'ok': True, 'item': r.to_dict()})


# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#  API â€” Requester: confirm sample received
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def api_receive(rid):
    ...
    _log(r, 'STATUS_CHANGE', from_status=prev, to_status='sample_received',
         note=f'Sample received. Qty: {r.received_qty or "â€”"}, '
              f'Batch: {r.batch_no or "â€”"}')

    # Notify Purchase that the loop is closed
    _notify(
        r, _purchase_user_ids(),
        title=f'{r.request_no} â€” sample received âœ…',
...
def api_cancel(rid):
    ...
    # No automatic self-cancel for requesters â€” must be granted explicitly.
    ...
    if r.status not in MUTABLE_STATUSES:
        return jsonify({
            'error': f'Cannot cancel â€” request is already "{r.status_label}".'
        }), 409
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#  API â€” soft delete (admin / manager only)
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def api_delete(rid):
    ...
    if r.status not in MUTABLE_STATUSES:
        return jsonify({
            'error': f'Cannot delete â€” request is "{r.status_label}". '
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#  API â€” BULK actions (multi-select on listing)
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#
...
def _parse_bulk_rows(data):
    ...
    Supports two payload formats for backward compatibility:
      ...
      2. Shared: {'ids': [1, 2, ...], 'foo': 'a'} â†’ same fields applied to all
...
def api_bulk_dispatch():
    ...
    for rid, f in rows:
        ...
        r.status           = 'order_dispatched'
        _log(r, 'STATUS_CHANGE', from_status=prev, to_status='order_dispatched',
             note=f'(Bulk) Dispatched via {courier or "â€”"}, '
                  f'tracking: {tracking or "â€”"}')
...
def api_bulk_cancel():
    ...
    for rid in ids:
        ...
        r.status = 'cancelled'
        _log(r, 'STATUS_CHANGE', from_status=prev, to_status='cancelled',
             note=f'(Bulk) Cancelled. Reason: {reason or "â€”"}')
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#  API â€” Export / Import / Column preferences
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def api_import_template():
    """Empty CSV with proper headers â€” for users to fill and upload."""
    if not _can_import():
    ...
    for i, row in enumerate(reader, start=2):   # row 2 = first data row
        ...
        try:
            ...
            _notify(
                ...
                body=f'{req.material_name} (qty: {req.quantity or "â€”"}) '
...
def api_columns():
    ...
    # GET â†’ return user's saved cols (or default)
...
def api_daily_reminder():
    ...
    # Role-based visibility â€” same rule as listing
    if not (_is_admin(current_user) or _is_purchase(current_user)):
        base = base.filter(RawMaterialSampleRequest.requested_by == current_user.id)
    ...
    # Find earliest creation date among pending â€” used in the popup banner
    earliest = None
    for r in rows:
        if r.created_at:
            d = r.created_at.date()
    ...
    for r in rows[:100]:    # cap for safety
        items.append({
            ...
            'material_name' : r.material_name or 'â€”',
            'status'        : r.status,
