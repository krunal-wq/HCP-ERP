"""
approval_routes.py â€” Hierarchy Management + Approval Workflow
"""
...
@login_required
def hierarchy():
    """Show full org chart â€” admin sees all, managers see their subtree."""
...
def set_manager():
    ...
    # Prevent circular â€” manager can't report to their own subordinate
    if manager_id:
    ...
    audit('approvals','SET_MANAGER', emp_id, emp.full_name, f'Manager set by {current_user.username}: {emp.full_name} â†’ {manager_id if manager_id else "None"}')
    db.session.commit()
...
@login_required
def approvals():
    """List approvals â€” pending ones needing my action at top."""
    my_emp = Employee.query.filter_by(user_id=current_user.id).first()
...
def approval_action(ar_id):
    ...
    # Permission check â€” admin can always act; others must be in approver chain
    if current_user.role != 'admin':
        my_emp = Employee.query.filter_by(user_id=current_user.id).first()
