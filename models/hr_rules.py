"""
models/hr_rules.py â€” Comprehensive HRMS Rules Engine

Tables:
  hr_shifts              â€” Shift master (Morning/Evening/Night/General)
  hr_locations           â€” Location master (Office/Factory/WFH)
  hr_late_rules          â€” Late Coming Rules (Location+Shift wise)
  hr_early_going_rules   â€” Early Going Rules
  hr_leave_policies      â€” Leave Policy master
  hr_leave_types         â€” Leave types per policy
  hr_lop_rules           â€” Loss of Pay rules
  hr_overtime_rules      â€” Overtime rules
  hr_compoff_rules       â€” Comp Off rules
  hr_absent_rules        â€” Absent penalty rules
"""
...
class HRLateRule(db.Model):
    """
    Late coming rules â€” location + shift + employee_type combination.
...
class HREarlyGoingRule(db.Model):
    """Early leaving rules â€” location + shift wise."""
    ...
    half_day_before = db.Column(db.String(5))                 # HH:MM â€” before this = half day
    absent_before   = db.Column(db.String(5))                 # HH:MM â€” before this = absent
...
class HROvertimeRule(db.Model):
    """Overtime calculation rules â€” location + shift wise."""
...
class HRLeavePolicy(db.Model):
    """Leave policy master â€” location + employee_type wise."""
...
class HRLOPRule(db.Model):
    """Loss of Pay rules â€” location + employee_type wise."""
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# LEAVE APPLICATION  (apply â†’ approve/reject â†’ balance deduct)
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
