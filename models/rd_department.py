...
_RD_NORMALIZED_SET = {
    ...
    'r & d',          # ampersand to "and" wala case â€” hum normalize nahi karte taki yeh alag se match ho
}


def _normalize(s):
    """
    Normalize karo string ko â€” lowercase, saari punctuation remove,
    ...
    x = x.replace('&', ' and ')
    # All non-alphanumeric â†’ space (. , _ - / etc.)
    x = re.sub(r'[^a-z0-9]+', ' ', x)
...
def is_rd_department(value):
    ...
    Examples of True:
        'R&D'                       â†’ 'r and d' â†’ matches
        'R & D'                     â†’ 'r and d' â†’ matches
        'Research and Development'  â†’ 'research and development'
        'Research & Development'    â†’ 'research and development'
        'RD'                        â†’ 'rd'
        'R and D'                   â†’ 'r and d'
        'R_and_D Dept'              â†’ 'r and d dept' â†’ contains 'r and d'
        'RND Team'                  â†’ 'rnd team' â†’ contains 'rnd'
    ...
    # Token-based match â€” jab 'r and d' / 'rnd' / 'research development'
    ...
    for kw in ('r and d', 'research and development', 'research development'):
        if kw in n:
            # Must be word-bounded â€” 'brand d' shouldn't match 'r and d'
    ...
    # Dotted variations like "R.N.D.", "R.D." â€” normalize se 'r n d' / 'r d' ban jaate hain
    if tokens == ['r', 'n', 'd'] or tokens == ['r', 'd']:
        return True
    # "R.N.D. Dept" â†’ "r n d dept" â†’ check prefix
...
def rd_department_filter(EmployeeModel, include_designation=False):
    ...
    `_normalize()` SQL me nahi hai, toh practical patterns cover karte hain:
      ...
      - 'rnd', 'rd'     (standalone â€” care kiya gaya hai word boundaries ka)
    """
    patterns = [
...
# â”€â”€ Quick self-test (run: python -m models.rd_department) â”€â”€
if __name__ == '__main__':
    ...
    for inp, expected in test_cases:
        ...
        mark = 'âœ…' if got == expected else 'âŒ'
        if got != expected: failed += 1
        print(f"  {mark} is_rd_department({inp!r:<40}) â†’ {got}  (expected {expected})")
