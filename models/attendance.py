"""
models/attendance.py â€” Attendance Models

Tables:
  raw_punch_logs  â€” Device se har punch aata hai (multiple per day per employee)
  attendance      â€” Daily summary: first punch_in + last punch_out
"""
...
class Attendance(db.Model):
    ...
    # WOP = Week Off Present â€” employee ne weekly off ke din punch kiya hai.
    ...
    def punch_in_str(self):
        return self.punch_in.strftime('%I:%M %p') if self.punch_in else 'â€”'
...
class HolidayMaster(db.Model):
    """
    Company holidays â€” Diwali, Holi, etc.
...
class LateShiftRule(db.Model):
    ...
    late_after      = db.Column(db.String(5), nullable=False)     # HH:MM â€” is time ke baad late
    # e.g. HCP OFFICE = 10:46
    half_day_after  = db.Column(db.String(5), nullable=True)      # HH:MM â€” is time ke baad half day
    absent_after    = db.Column(db.String(5), nullable=True)      # HH:MM â€” is time ke baad absent
...
class LatePenaltyRule(db.Model):
    ...
    # Time band â€” punch_in ka time kis range mein hai
    ...
    # Late count band â€” is mahine mein kitni baar late aaya
    ...
    def count_range_label(self):
        ...
        return f'{self.from_count} â€“ {self.to_count} baar'

    def time_range_label(self):
        if self.time_to:
            return f'{self.time_from} â€“ {self.time_to}'
...
class EarlyComingRule(db.Model):
    ...
    employee_type   = db.Column(db.String(100), nullable=False, unique=True)
    shift_start     = db.Column(db.String(5), default='09:00')    # HH:MM
    early_before    = db.Column(db.String(5), nullable=False)      # HH:MM â€” is se pehle aaye = early
    # e.g. 08:45 â€” 08:45 se pehle aana = early coming
