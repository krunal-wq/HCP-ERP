"""
rd_sample_log_routes.py â€” R&D Sample Log Menu
============================================
...
# Full visibility â€” all projects, all employees
FULL_ACCESS_ROLES = {'admin', 'manager', 'npd_manager', 'rd_manager'}

# Mid-tier â€” all rows on THEIR projects (Team Lead / NPD / R&D executive)
PROJECT_SCOPED_ROLES = {'rd_executive', 'sales', 'lead', 'team_lead', 'hr', 'npd'}

# Everyone else â†’ employee bucket (own rows only, on own projects)
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#  Core helper â€” projects visible to a user
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

def get_accessible_project_ids(user) -> list[int] | None:
    """
    Return a list of project IDs the given user may see.
    Returns None (meaning NO filter â€” see everything) for full-access roles.
    Returns [] if the user has no assignments (â†’ empty page).
    """
...
def index():
    ...
    """
    # â”€â”€ Permission gate â€” the menu itself must be viewable â”€â”€
    perm = get_perm('rd_sample_log')
    if perm is None or (perm and not getattr(perm, 'can_view', True)):
        # Fallback â€” admins bypass, everyone else gets a polite 403 page
    ...
    # â”€â”€ Build the sample-log query â”€â”€
    # Exclude 'sent_to_office' rows â€” they live in the separate listing page
    ...
    # member filter â€” only full-access & project-scoped roles may filter
    ...
    # (.nullslast() is Postgres-only â€” MySQL rejects 'NULLS LAST' syntax.)
    logs = q.order_by(
        (RDSubAssignment.started_at.is_(None)).asc(),   # False(0) before True(1) â†’ non-null first
    ...
    # â”€â”€ Summary counts (based on already-scoped query, minus UI filters) â”€â”€
    # Also excludes 'sent_to_office' â€” mirrors main query
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#  JSON endpoint â€” for AJAX refresh / mobile / integrations
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#  DIAGNOSTIC endpoint â€” helps debug "why count is X?" issues
...
def api_debug():
    ...
    projects_detail = []
    if accessible_pids is None:
        # Full access â€” just count total, don't list all
        total = NPDProject.query.filter_by(is_deleted=False).count()
        projects_detail = [{'note': f'Full access â€” all {total} projects visible'}]
    else:
    ...
    return jsonify({
        ...
        'note': (
            ...
            'If you expected a higher number, check projects_detail â€” '
...
#  SAMPLE CODE PERSISTENCE â€” added for Print Sticker workflow
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#  Endpoints:
#    POST  /rd/sample-log/api/generate-codes       â†’ auto-fill blanks (no save)
#    POST  /rd/sample-log/api/save-sample-codes    â†’ validate + persist before print
#
...
# 1. GENERATE CODES â€” auto-fill blank sample codes in modal (no save)
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def api_generate_codes():
    ...
    for r in rows:
        # Skip rows that already have a code â€” don't clobber
...
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# 2. SAVE SAMPLE CODES â€” persist BEFORE print
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def api_save_sample_codes():
    ...
    Behaviour:
      ...
      - If anything fails â†’ nothing is saved, error returned.
      - Otherwise â†’ each row's `sample_code` field is overwritten with
    ...
    # â”€â”€ 1. Normalise & basic validation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    cleaned = []
    for item in payload:
        try:
            rid = int(item.get('id'))
    ...
    # â”€â”€ 2. In-batch duplicate check (case-insensitive) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    seen_in_batch = {}   # upper-code â†’ row_id
    batch_dups = []
    for r in cleaned:
        for c in r['codes']:
            key = c.upper()
    ...
    # â”€â”€ 3. Fetch existing rows & permission check â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # Access control â€” employees can only save their OWN rows
    bucket = _user_bucket(current_user)
    if bucket == 'employee':
    ...
    # â”€â”€ 4. DB-wide duplicate check (exclude rows being edited) â”€â”€
    ...
    # â”€â”€ 5. Persist â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    saved = []
    for r in cleaned:
        row = rows_by_id[r['id']]
...
def _apply_scope(q, user):
    """Re-usable project-scope filter â€” mirror of the logic in index()."""
...
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# 1. SEND TO OFFICE â€” save codes + flip status
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def api_send_to_office():
    ...
    Steps (atomic):
      1. Validate & save sample codes (reuses the same rules as
         api_save_sample_codes â€” empty/dup/permission).
    ...
    # Persist â€” save codes + flip status + stamp dispatch metadata
    ...
    # office â†’ Status should change to Sent to Office".
...
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# 2. REVERT â€” sent_to_office â†’ finished (sample ready)
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def api_revert(row_id):
    ...
    # Re-aggregate project status â€” when this row reverts, project may go
...
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# 3. SENT-TO-OFFICE LISTING â€” redirect to existing NPD Sample History
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
#  DIAGNOSE & ACTIVATE â€” recovery utilities
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
#
#  Usage:
#    â€¢ /rd/sample-log/api/diagnose  (GET, no args)
#        â†’ returns counts + samples explaining exactly what's visible/hidden
#
#    â€¢ /rd/sample-log/api/activate?scope=all   (POST, admin-only)
#        â†’ reactivates ALL rd_sub_assignments:
#           is_active = TRUE
#           status    = 'finished' (if currently 'sent_to_office')
#           clears send_to_office_date / sent_to_office_by
#        â†’ also deletes orphan OfficeDispatchItems linked to them
#
#    â€¢ /rd/sample-log/api/activate?scope=mine  (POST)
#        â†’ same, but only for the current user's rows
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def api_diagnose():
    ...
    return jsonify({
        ...
        'hint': (
            ...
            '(a) your rows are all sent_to_office â€” check /rd/sample-history; '
            '(b) your rows have is_active=0 â€” run POST /api/activate to fix; '
            '(c) the projects are not assigned to you â€” ask admin; '
...
def api_activate():
    ...
    Query params:
      ?scope=mine   (default) â€” only current user's rows
      ?scope=all              â€” every row (ADMIN / MANAGER only)

    What it does for each affected row:
      â€¢ is_active              â†’ True
      â€¢ status                 â†’ 'finished'        (only if currently 'sent_to_office')
      â€¢ send_to_office_date    â†’ NULL
      â€¢ sent_to_office_by      â†’ NULL
