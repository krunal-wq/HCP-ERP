...
Routes:
  GET  /mail/master              â€” List all email templates
  GET  /mail/master/<code>/edit  â€” Edit template form
  POST /mail/master/<code>/edit  â€” Save template
  POST /mail/leads/<id>/send-npd â€” Send NPD email to lead
  GET  /mail/leads/<id>/preview-npd â€” Preview NPD email (modal)
"""
...
# â”€â”€ Default NPD template body â”€â”€
...
<div>To facilitate a seamless payment process, we have provided payment links below: <a href="https://www.hcpwellness.in/our-bank-detail/"><strong>â‚¹ Payment Link</strong></a> (Click to pay through Online / UPI / QR Code / Bank Details)</div>
<br />
<div>= = = = = = = = = = = = = = = = = = = = = = = = = = =</div><br />
...
ðŸ“± <a href="https://wa.me/+919723455627"><strong>WhatsApp for quick communication</strong></a><br />
âœ‰ info@hcpwellness.in | ðŸŒ www.hcpwellness.in</div>"""

NPD_DEFAULT_SUBJECT = "{company} - Request for proposal (RFP) for Skincare products"


# â”€â”€ Default Sample Order template body â”€â”€
...
# â”€â”€ Default Sample Dispatch (to client) Email template â”€â”€
# Used by Client Dispatch module â€” sends approved samples to client with
...
#   {sample_lines_html}        â† <li>code â€“ product</li>... bullet list
#   {sender_name}, {extra_notes}
SAMPLE_DISPATCH_DEFAULT_SUBJECT = "Sample Dispatch Details â€“ {project_code}"
...
# â”€â”€ Default Quotation Email template â”€â”€
...
def _get_or_create_sample_dispatch_template():
    """Get Sample Dispatch (to client) template â€” created on first access.
...
def _render_sample_dispatch_vars(text, project, items, courier_name,
    ...
    for it in items:
        ...
        sample_lines_html += f'<li style="margin:4px 0;">{code} â€“ {product}</li>'

    # Build tracking line â€” show BOTH courier and tracking number when
...
def _build_quotation_items_table(quot):
    ...
    for it in items:
        def _v(k, fmt='str'):
            val = it.get(k, '')
            if fmt == 'num':
                try: v = float(val); return f'{v:,.2f} INR' if v else 'â€”'
                except: return 'â€”'
            if fmt == 'int':
                try: return str(int(float(val))) if val else 'â€”'
                except: return 'â€”'
            return str(val) if val else 'â€”'
        rows += (
            f'<tr>'
            f'<td {td.format(a="left")}>{_v("name")}</td>'
            f'<td {td.format(a="center")}>{(_v("size") + " " + _v("uom")).strip() if (_v("size") != "â€”" or _v("uom") != "â€”") else "â€”"}</td>'
...
# MAIL MASTER â€” List & Edit templates
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def mail_master():
    ...
    if not get_sub_perm('crm', 'mail_master'):
...
def mail_template_edit(code):
    ...
    if request.method == 'POST':
        ...
        flash(f'âœ… Template "{t.name}" saved successfully!', 'success')
...
def lead_send_npd(id):
    ...
    if not to_email:
        flash('âŒ Email address required!', 'danger')
    ...
    if success:
        ...
        db.session.commit()
        audit('mail', 'SEND', id, f'NPD â†’ {to_email}', obj=None)
        flash(f'âœ… NPD Email sent successfully to {to_email}!', 'success')
    else:
        flash(f'âŒ Email failed: {err}', 'danger')
...
def lead_send_email(id):
    """Quick custom email to a lead. AJAX endpoint â€” takes JSON
    ...
    if success:
        try:
            ...
            db.session.rollback()
        try:
            audit('mail', 'SEND', id, f'Email â†’ {to_email}', obj=None)
        except Exception:
            pass
...
# SEND SAMPLE ORDER EMAIL â€” from SampleOrder
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
def sample_order_send(id):
    ...
    GET  â€” direct one-click send using DB template (no form needed)
    POST â€” send with optional overrides from modal form
    """
    ...
    # Auto-render from template â€” works even if form fields are empty
    ...
    if not to_email:
        flash(f'âŒ No email found for order {so.order_number}. Please add email to lead/order first.', 'danger')
        return back
    ...
    if success:
        ...
        db.session.commit()
        audit('mail', 'SEND', so.id, f'SampleOrder {so.order_number} â†’ {to_email}', obj=None)
        flash(f'âœ… Sample Order email sent to {to_email} ({so.order_number})!', 'success')
    else:
        flash(f'âŒ Email failed for {so.order_number}: {err}', 'danger')

    return back


# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# QUICK SEND â€” one-click from any page
...
def sample_order_quick_send(id):
    ...
    if not to_email:
        flash(f'âŒ No email address for order {so.order_number}. Update the lead email first.', 'danger')
    ...
    if success:
        ...
        db.session.commit()
        audit('mail', 'SEND', so.id, f'SampleOrder {so.order_number} â†’ {to_email}', obj=None)
        flash(f'âœ… Sample Order email sent to {to_email}!', 'success')
    else:
        flash(f'âŒ Email failed: {err}', 'danger')
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# QUOTATION EMAIL â€” from Quotations list
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
